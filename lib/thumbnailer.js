...
Thumbnailer.prototype.execute = function(description, localPaths, onComplete) {
	...
	_.extend(this, {
		...
		autoOrient: (decsription.autoOrient || null),
...
Thumbnailer.prototype.matted = function() {
	var dimensionsString = this.width + 'X' + this.height,
	  qualityString = (this.quality ? '-quality ' + this.quality : ''),
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + autoOrient + ' -resize ' + dimensionsString + ' -size ' + dimensionsString + ' xc:' + this.background + ' +swap -gravity center' + qualityString + ' -composite ' + this.convertedPath;
...
Thumbnailer.prototype.bounded = function() {
	var dimensionsString = this.width + 'X' + this.height,
		qualityString = (this.quality ? '-quality ' + this.quality + ' ' : ''),
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + autoOrient + ' -thumbnail ' + dimensionsString + ' ' + qualityString + this.convertedPath;
...
Thumbnailer.prototype.fill = function() {
	var dimensionsString = this.width + 'X' + this.height,
		qualityString = (this.quality ? '-quality ' + this.quality : ''),
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + autoOrient + ' -resize ' + dimensionsString + '^ -gravity center -extent ' + dimensionsString + ' ' + qualityString + ' ' + this.convertedPath;
...
Thumbnailer.prototype.strict = function() {
	var dimensionsString = this.width + 'X' + this.height,
		qualityString = (this.quality ? '-quality ' + this.quality : ''),
		autoOrient = (this.autoOrient ? '-auto-orient' : ''),
		thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '[0]" ' + autoOrient + ' -resize ' + dimensionsString + '! ' + qualityString + ' ' + this.convertedPath;
