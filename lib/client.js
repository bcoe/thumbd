 ...
 */
Client.prototype.thumbnail = function(originalImagePath, thumbnailDescriptions, opts, callback) {
	/**
		job = {
			"resources": [
				"/foo/awesome.jpg"
	...
	*/
	...
	if (!_.isArray(originalImagePath)) originalImagePath = [originalImagePath];

	// override defaults with opts.
	opts = _.extend({
		prefix: originalImagePath[0].split('.').slice(0, -1).join('.')
	}, opts);
