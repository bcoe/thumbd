 ...
 */
Grabber.prototype.download = function(bucket, remoteImagePath, callback) {
	var _this = this,
	...
	tmp.file({dir: config.get('tmpDir'), postfix: "." + extension}, function(err, localImagePath, fd) {
		...
		if (remoteImagePath.match(/https?:\/\//)) { // we are thumbnailing a remote image.
			...
			_this.getFileS3(bucket, remoteImagePath, localImagePath, stream, callback);
		}
 ...
 * @param function callback The callback function
 */
Grabber.prototype.getFileS3 = function(bucket, remoteImagePath, localImagePath, stream, callback) {
	var _this = this,
		req = utils.s3(bucket).getFile(remoteImagePath, function(err, res) {
